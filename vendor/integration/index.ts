  ...
  return {
    ...
    hooks: {
      'astro:config:setup': async ({
        ...
        const buildLogger = logger.fork('MedSyntra');

        const virtualModuleId = 'MedSyntra:config';
        ...
        updateConfig({
          ...
          vite: {
            plugins: [
              {
                name: 'vite-plugin-MedSyntra-config',
                resolveId(id) {
        ...
        if (typeof _themeConfig === 'string') {
          ...
          buildLogger.info(`MedSyntra \`${_themeConfig}\` has been loaded.`);
        } else {
          buildLogger.info(`MedSyntra config has been loaded.`);
        }
      },
